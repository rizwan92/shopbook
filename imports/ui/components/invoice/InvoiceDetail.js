...
import { withRouter } from 'react-router-dom'
 ...
 class InvoiceDetail  extends Component {
  ...
  componentDidMount(){
    ...
    Meteor.call('invoice.get',this.props.match.params.id,function(error,result){
        setMyState(result);
    });
  }
  myNavigation(){
    ...
    return(
       ...
       <div id="divcontents">
         <div style={{display:'flex',flex:1,flexFlow:'column'}}>
           ...
           <div style={{paddingLeft:50,fontSize:13,}}>{this.state.invoice.to}</div>
           <div style={{paddingLeft:50,fontSize:13,}}>{this.state.invoice.tonumber}</div>
           </div>
         </div>
